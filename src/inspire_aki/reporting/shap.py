...
def generate_shap_outputs(artifacts: ArtifactManager, config: dict) -> list[Path]:
    ...
    for job in jobs:
        ...
        if bundle.scaler is not None:
            x_background = pd.DataFrame(bundle.scaler.transform(x_background), columns=feature_cols)
            x_explain = pd.DataFrame(bundle.scaler.transform(x_explain), columns=feature_cols)
