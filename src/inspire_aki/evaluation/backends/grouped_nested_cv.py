...
from inspire_aki.evaluation.split_manager import build_grouped_nested_cv_manifest


@dataclass
class GroupedNestedCVBackend:
    config: dict
    mode: str = "grouped_nested_cv"
    ...
    def build(self, df: pd.DataFrame, *, target: str, dataset_family: str) -> EvaluationBackendResult:
        bundle = build_grouped_nested_cv_manifest(
            ...
            outer_n_splits=int(self.config["splits"]["n_cv_folds"]),
