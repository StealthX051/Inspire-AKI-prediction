from __future__ import annotations

from dataclasses import dataclass

import numpy as np
import pandas as pd
from joblib import Parallel, delayed

from inspire_aki.runtime import build_stage_runtime_plan, thread_limited_context


def calculate_net_benefit_for_thresholds(y_true: np.ndarray, y_prob: np.ndarray, pt_grid: np.ndarray) -> np.ndarray:
    benefits = []
    for pt in pt_grid:
        safe_pt = min(float(pt), 0.99999)
        y_pred = y_prob >= safe_pt
        tp = np.sum((y_pred == 1) & (y_true == 1))
        fp = np.sum((y_pred == 1) & (y_true == 0))
        n = len(y_true)
        benefits.append((tp / n) - (fp / n) * (safe_pt / (1 - safe_pt)))
    return np.asarray(benefits)


@dataclass
class DecisionCurveResult:
    curves: pd.DataFrame
    bootstrap_ci: pd.DataFrame


def _bootstrap_dca_ci(
    y_true: np.ndarray,
    y_prob: np.ndarray,
    pt_grid: np.ndarray,
    *,
    n_bootstrap: int,
    random_state: int,
) -> tuple[np.ndarray, np.ndarray]:
    if n_bootstrap < 1 or len(y_true) == 0:
        nan_values = np.full(len(pt_grid), np.nan, dtype=float)
        return nan_values, nan_values
    rng = np.random.default_rng(random_state)
    samples: list[np.ndarray] = []
    n_samples = len(y_true)
    for _ in range(n_bootstrap):
        idx = rng.integers(0, n_samples, n_samples)
        y_boot = y_true[idx]
        if len(np.unique(y_boot)) < 2:
            continue
        p_boot = y_prob[idx]
        samples.append(calculate_net_benefit_for_thresholds(y_boot, p_boot, pt_grid))
    if not samples:
        nan_values = np.full(len(pt_grid), np.nan, dtype=float)
        return nan_values, nan_values
    stacked = np.vstack(samples)
    return np.nanpercentile(stacked, 2.5, axis=0), np.nanpercentile(stacked, 97.5, axis=0)


def _decision_curve_group_worker(
    keys: tuple,
    group_df: pd.DataFrame,
    pt_grid: np.ndarray,
    *,
    n_bootstrap: int,
    random_state: int,
    nested_blas_threads: int,
) -> tuple[list[dict[str, object]], list[dict[str, object]]]:
    with thread_limited_context(nested_blas_threads):
        y_true = group_df["y_true"].astype(int).to_numpy()
        y_prob = group_df["y_prob_calibrated"].fillna(group_df["y_prob_raw"]).astype(float).to_numpy()
        prevalence = float(np.mean(y_true))
        model_nb = calculate_net_benefit_for_thresholds(y_true, y_prob, pt_grid)
        treat_all = prevalence - (1 - prevalence) * (pt_grid / (1 - pt_grid))
        threshold_optimal = float(group_df["threshold"].iloc[0]) if "threshold" in group_df.columns and not group_df.empty else np.nan
        ci_lower, ci_upper = _bootstrap_dca_ci(
            y_true,
            y_prob,
            pt_grid,
            n_bootstrap=n_bootstrap,
            random_state=random_state,
        )
        point_rows: list[dict[str, object]] = []
        bootstrap_rows: list[dict[str, object]] = []
        for threshold, net_benefit_model, net_benefit_all, lower_ci, upper_ci in zip(
            pt_grid,
            model_nb,
            treat_all,
            ci_lower,
            ci_upper,
        ):
            common_row = {
                "dataset_regime": keys[0],
                "population_id": keys[1],
                "model_key": keys[2],
                "threshold_prob": float(threshold),
                "net_benefit_model": float(net_benefit_model),
                "net_benefit_treat_all": float(net_benefit_all),
                "net_benefit_treat_none": 0.0,
                "threshold_optimal": threshold_optimal,
            }
            point_rows.append(common_row)
            bootstrap_rows.append(
                {
                    **common_row,
                    "ci_lower_95": float(lower_ci) if np.isfinite(lower_ci) else np.nan,
                    "ci_upper_95": float(upper_ci) if np.isfinite(upper_ci) else np.nan,
                }
            )
    return point_rows, bootstrap_rows


def decision_curve_outputs(predictions_df: pd.DataFrame, config: dict) -> DecisionCurveResult:
    if predictions_df.empty:
        return DecisionCurveResult(curves=pd.DataFrame(), bootstrap_ci=pd.DataFrame())

    eval_cfg = config["evaluation"]
    pt_grid = np.arange(
        eval_cfg["dca_threshold_min"],
        eval_cfg["dca_threshold_max"] + eval_cfg["dca_threshold_step"] / 2,
        eval_cfg["dca_threshold_step"],
    )

    group_cols = ["dataset_regime", "population_id", "model_key"]
    groups = [(keys, group_df.copy()) for keys, group_df in predictions_df.groupby(group_cols, sort=False)]
    runtime_plan = build_stage_runtime_plan(config, "evaluate_dca", {"group_count": len(groups)})
    rows_nested = Parallel(n_jobs=max(1, runtime_plan.evaluation_workers), backend="loky")(
        delayed(_decision_curve_group_worker)(
            keys,
            group_df,
            pt_grid,
            n_bootstrap=int(eval_cfg.get("bootstrap_reps", 0)),
            random_state=int(config["splits"]["random_state"]) + group_idx,
            nested_blas_threads=runtime_plan.nested_blas_threads,
        )
        for group_idx, (keys, group_df) in enumerate(groups)
    )
    point_rows = [row for point_rows, _ in rows_nested for row in point_rows]
    bootstrap_rows = [row for _, bootstrap_rows in rows_nested for row in bootstrap_rows]
    return DecisionCurveResult(curves=pd.DataFrame(point_rows), bootstrap_ci=pd.DataFrame(bootstrap_rows))


def decision_curve_table(predictions_df: pd.DataFrame, config: dict) -> pd.DataFrame:
    return decision_curve_outputs(predictions_df, config).curves
