...
- A maintained pipeline whose shipped configs use patient-grouped evaluation modes to reduce patient-overlap leakage, and whose grouped calibration and thresholding are now learned from outer-train OOF predictions generated by patient-grouped inner folds.
...
- Do not describe the archived operation-level repeated-CV workflow as the recommended current process; the maintained default is patient-grouped evaluation plus outer-train-only grouped calibration and thresholding.
