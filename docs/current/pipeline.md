...
- The focused reviewer-response scripts `scripts/department_os_audit.py` and `scripts/department_ot_reviewer_report.py` are maintained utilities for narrow department provenance and ophthalmology subgroup questions. They are intentionally outside `run all` and the CLI stage map, and they now default to artifact-root markdown/CSV outputs under `reports/reviewer_department_audit/`.
- The maintained reviewer-response utility `scripts/run_reviewer_missingness_sensitivity.sh` reruns a fresh combined `xgb` baseline and then calls `scripts/combined_xgb_missingness_sensitivity.py` for the reviewer-specific missing-data sensitivity analysis. It is intentionally outside `run all` because the alternative imputation path, calibration fit, and threshold selection must stay inside grouped outer folds rather than be applied as a global preprocess toggle, and its comparison outputs now default to the chosen sensitivity artifact root under `reports/reviewer_missingness_sensitivity/`.
