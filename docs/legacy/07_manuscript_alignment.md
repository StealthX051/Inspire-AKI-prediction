...
| Zero-duration operations | Brief does not call out zero-length surgeries | raw `operations.csv`, refactor `src/inspire_aki/cohort/filters.py` | Legacy scripts allowed `op_len == 0`; refactor now excludes `op_len <= 0` before downstream feature engineering | high | Small cohort-count drift is now expected between legacy-style counts and refactor runs |
...
| Calibration and F2 thresholding | Brief expects isotonic calibration and fixed F2-optimal thresholds | `create_results/13_performance_metrics.ipynb`, `create_results/bootstrap_metrics.py`, `create_results/decision_curve.py` | Current notebook applies isotonic calibration, reselects F2-optimal thresholds, bootstraps metrics, and generates DCA; the refactor now keeps repeated rows for the same `op_id` together during calibration CV | high | Legacy notebook-style calibration pooled repeated rows without group protection, so minor calibration drift between legacy and refactor is now expected |
