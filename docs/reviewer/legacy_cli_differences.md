...
| Reporting surface | Archived reporting depended heavily on notebooks | The CLI owns manuscript tables, curves, consort, DeLong, DCA, reclassification, and optional SHAP outputs including maintained raw-value scatter plots and opt-in manual dependence plots; the maintained scatter/dependence path now also explains the full held-out split while keeping a bounded background sample | Publication-facing outputs are now generated from stage-owned artifacts instead of notebook-local saved figures while preserving notebook-style SHAP point coverage |
