...
| Drift | Legacy behavior | Refactored behavior | Why |
| --- | --- | --- | --- |
| Label artifact contract | Legacy `04_AKI_data_selection.py` derived `aki_boolean` and wrote only the labeled combined, preop, and intraop tabular datasets | Refactor also writes standalone `cohort/aki_labels.csv` and `cohort/labels_audit.csv`, plus a stage manifest, before joining labels back onto the tabular datasets under the configured artifact root | Makes label reuse and cohort-step counts explicit for downstream timeseries/sequence filtering, reporting, and handoff |
...
| Timeseries/sequence staging | Earlier refactor wrote only final `time_series_cleaned.csv` and `lstm_trainable.pkl` outputs | The refactor now also writes internal partitioned staging artifacts under `<artifacts_dir>/staging/` | Supports safe multicore preprocessing without changing the top-level output contract |
...
| HPO result artifact contract | Legacy HPO wrote copy-paste text summaries such as `tabular_hpo_results.txt` and `hybrid_hpo_results.txt`; detailed trial history was not persisted as a structured artifact | Refactor writes machine-readable `tabular_best_params.json`, `sequence_best_params.json`, `tabular_trials.parquet`, `sequence_trials.parquet`, and stage manifests under the configured artifact root | Makes tuning outputs auditable, diffable, and reusable by downstream stages without manual transcription |
...
| AutoGluon CPU/GPU budgeting | Legacy `08_tabular_model_creation.py` relied on AutoGluon defaults and did not pass explicit host budgets | Refactor passes `models.autogluon.num_cpus` and `models.autogluon.num_gpus` into `TabularPredictor.fit(...)`; on the current 32-CPU host the main default config pins `num_cpus: 32` and leaves `num_gpus: auto` | Makes AutoGluon resource use explicit on this host without changing the model family or adding AutoGluon to Optuna HPO |
...
| Summary metric aggregation | Legacy performance tables in `13_performance_metrics.ipynb` summarized per-fold metric arrays from the result pickles and reported means plus t-based confidence intervals | Refactor still writes pooled `metrics_summary.csv` for machine-readable evaluation, but manuscript tables are now rebuilt from per-fold/per-run metrics and rendered with legacy-style point estimates plus t-interval presentation | Keeps a thin pooled evaluation artifact while restoring the legacy manuscript-facing aggregation contract |
...
| DeLong multiple-testing correction | Legacy `14_delong_table.ipynb` produced both raw p-value tables and a Benjamini-Hochberg FDR-corrected table / CSV | Refactor `evaluate delong` now also writes `evaluation/delong_fdr_corrected.csv` and `evaluation/delong_fdr_corrected_long.csv`, and manuscript reporting renders both raw and corrected tables with the legacy exclusion policy | Restores the corrected significance view without reintroducing notebook-specific pickle dependencies |
...
| DCA artifact and plotting contract | Legacy `decision_curve.py` generated one per-model DCA plot plus one per-model DCA CSV, bootstrapped 95% CI bands, and marked the calibrated F2-optimal threshold `tau_star` on the plot | Refactor `evaluate dca` now writes both `evaluation/dca_curves.csv` and `evaluation/dca_bootstrap_ci.csv`; downstream reporting restores per-model DCA plots with 95% CI bands, treat-all / treat-none references, `tau_star`, and cross-dataset comparison figures | Keeps evaluation artifacts stage-owned while regaining the manuscript-facing DCA view |
...
| Drift | Legacy behavior | Refactored behavior | Why |
| --- | --- | --- | --- |
| Consort count source and step semantics | Legacy `11_consort.ipynb` recomputed cohort counts directly from raw `operations.csv`, `labs.csv`, `ward_vitals.csv`, and intraop feature availability, then emitted manuscript-oriented steps with explicit exclusion reasons such as missing intraop variables and missing post-op outcome | Refactor now still reuses stage-owned `cohort/preop_audit.csv` and `cohort/labels_audit.csv`, but folds them into manuscript-style mainline steps plus grouped exclusion summaries for preop filtering, missing intraop features, and labeling exclusions | Keeps the report stage stage-owned while restoring a manuscript-facing consort contract |
| Consort intraop and final AKI split coverage | Legacy `11_consort.ipynb` included an explicit `Operations after excluding missing intraoperative variables` step and appended final negative/positive AKI case counts for the terminal split | Refactor now derives the missing-intraop exclusion summary, carries final labeled cohort counts forward, and branches the terminal diagram into `No postoperative AKI` and `Postoperative AKI` nodes | Restores the expected terminal split without rerunning the full raw-data notebook logic |
| Consort DOT graph contract | Legacy `11_consort.ipynb` wrote a vertical DOT graph with exclusion annotation nodes and a final branch to negative / positive AKI cases | Refactor now writes a top-down Graphviz DOT graph with explicit mainline/exclusion edges and direct orthogonal terminal arrows from the final labeled cohort to the AKI / non-AKI nodes, then renders `consort.png` / `consort.svg` from that DOT source | Aligns the graph surface with the manuscript convention while retaining machine-readable audit outputs |

### `report tables`

| Drift | Legacy behavior | Refactored behavior | Why |
| --- | --- | --- | --- |
| Performance-table contract | Legacy `13_performance_metrics.ipynb` wrote manuscript-style performance tables with `preop` / `intraop` / `combined` sections and one point-estimate row plus one CI row per model, and the checked-in repo also preserves a separate calibrated Markdown table | Refactor now rebuilds both `performance_table.*` and `performance_table_calibrated.*` from corrected prediction artifacts, restores sectioned manuscript formatting, enforces a fixed manuscript model order, omits `ASA Rule` outside preop, and emits each table as `html`, `md`, and `csv` | Restores legacy-style presentation while keeping the corrected refactor prediction source of truth |
| Performance-table HTML emphasis | Legacy notebook outputs relied on ordering and two-row CI presentation rather than browser-specific styling | Refactor HTML tables now add subtle monochrome per-cell washes whose intensity is driven column-wise, plus bold best-in-column values, so relative performance remains readable without changing the Markdown or CSV contract | Adds manuscript-friendly HTML emphasis while preserving machine-readable exports |
| Cohort-characteristics scope and granularity | Legacy `12_cohort_characteristics.ipynb` grouped to one row per `subject_id`, built a single overall Table 1, and included rows such as female sex, postoperative AKI prevalence, ASA class counts, and department counts | Refactor now rebuilds a manuscript-style overall cohort table from unnormalized tabular data, preop features, and labels, including ASA and department breakdowns, and emits it in `html`, `md`, and `csv` | Restores the legacy descriptive table layout while reusing stage-owned artifacts |
| Cohort-characteristics row semantics | Legacy `12_cohort_characteristics.ipynb` formatted binary rows as labeled counts plus percentages, such as `Female sex, n (%)`, and used pandas default sample SD for continuous summaries | Refactor now emits manuscript-style labeled count/percentage rows such as `Female sex, n (%)` and uses sample SD formatting for continuous summaries | Restores the domain-specific Table 1 presentation while keeping stage-owned inputs |
| Fill-rate table shaping | Legacy `12_cohort_characteristics.ipynb` built the fill-rate table from `_isna` columns, consolidated repeated regular-signal features to one row per physiologic label, stripped prefixes like `mean_` / `sum_` / `max_` / `min_`, and rendered a titled manuscript-style HTML table | Refactor now rebuilds the fill-rate table from `_isna` columns when available, consolidates repeated signal summaries, and emits manuscript-facing `html`, `md`, and `csv` outputs | Restores the notebook-specific presentation logic while keeping the table stage-owned |

### `report curves`

| Drift | Legacy behavior | Refactored behavior | Why |
| --- | --- | --- | --- |
| ROC / PR curve construction | Legacy `13_performance_metrics.ipynb` built mean ROC and PR curves from per-fold prediction arrays, optionally shaded them with foldwise standard-deviation bands, sorted legends by mean AUC, and labeled each curve with `mean ± std` AUC text | Refactor `reporting/curves.py` now rebuilds foldwise ROC and PR curves from corrected predictions, interpolates onto shared grids, adds `±1 SD` bands, and emits legend statistics in both `png` and `svg` | Restores the legacy manuscript-facing curve contract while staying on top of the refactor prediction artifacts |
| Calibration curve construction | Legacy `13_performance_metrics.ipynb` pooled per-model predictions from the result pickles and called `calibration_curve(..., n_bins=15, strategy='uniform')` on the stored model probabilities | Refactor now plots calibration curves from corrected calibrated probabilities with the same `n_bins=15` / `strategy='uniform'` contract and emits both `png` and `svg` | Restores the legacy visual contract without reverting the grouped calibration fix |
| Curve artifact scope and layout | Legacy curve generation wrote ROC / PR / calibration plots under `figures/curves/`, and the DCA workflow additionally wrote one per-model DCA figure plus cross-dataset DCA comparison figures under `figures/dca/` | Refactor now writes one ROC, PR, and calibration figure per `dataset_regime`, plus one per-model DCA figure and one per-model cross-dataset DCA comparison figure under `reports/figures/`, all in `png` and `svg` | Aligns the report stage with the legacy manuscript surface while keeping refactor-owned artifact paths |

### `report manuscript`

| Drift | Legacy behavior | Refactored behavior | Why |
| --- | --- | --- | --- |
| Manuscript reporting | Refactor initially required a separate SHAP stage even when the user asked for `report manuscript` | `report manuscript` now dispatches `consort`, `tables`, `curves`, `statistics`, `reclassification`, and `shap` from `reports.manuscript_sections`, with supplemental sections controlled by `reports.generate_supplemental_outputs` | Makes the report contract manuscript-complete and explicit in config |
...
| `run all` observability and overlap | Legacy scripts printed progress continuously, while the earlier refactor `run all` emitted only a final JSON blob after every stage finished serially | `run all` now emits immediate stage start/end events to stdout and `<artifacts_dir>/logs/run_all_events.jsonl`, and in `runtime.orchestration.mode: overlap` it overlaps `tune sequence` with `train tabular` after `tune tabular` | Restores live monitoring and improves whole-node utilization without adding a general scheduler |
