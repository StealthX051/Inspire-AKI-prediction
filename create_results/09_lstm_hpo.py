...
from sklearn.model_selection import train_test_split
from sklearn.preprocessing import StandardScaler
from sklearn.metrics import average_precision_score as auprc  # <-- AUPRC
...
def _safe_auprc(y_true, scores):
    """Return AUPRC; if y_true has a single class, return 0.0 to avoid errors."""
    y_true = np.asarray(y_true)
    if y_true.min() == y_true.max():  # only one class present
        return 0.0
    return auprc(y_true, scores)

def objective_builder(model_name, train_loader, val_loader, tabular_input_size, lstm_input_size, device):
    """Builds the Optuna objective function for a given model (metric = AUPRC)."""
    def objective(trial):
        # --- Suggest Hyperparameters ---
        lr = trial.suggest_float('lr', *search_spaces[model_name]['lr'], log=True)
        dropout_rate = trial.suggest_float('dropout_rate', *search_spaces[model_name]['dropout_rate'])
        ...
        best_val_metric = 0.0  # now stores best AUPRC
        patience_counter = 0
        ...
        for epoch in range(150):
            ...
            # --- Validation (AUPRC) and Early Stopping ---
            model.eval()
            all_val_probs = []
            with torch.no_grad():
                for batch in val_loader:
            ...
            val_probs = torch.cat(all_val_probs).numpy().flatten()
            y_val_np = val_loader.dataset.tensors[3].numpy()

            current_val_metric = _safe_auprc(y_val_np, val_probs)
...
# =============================================================================
def main():
    """Main function to run the HPO process (metric = AUPRC)."""
    torch.manual_seed(RANDOM_STATE)
    np.random.seed(RANDOM_STATE)
    optuna.logging.set_verbosity(optuna.logging.WARNING)
    ...
    for model_name, should_run in hpo_configs.items():
        ...
        # --- Run Optuna Study (maximize AUPRC) ---
        study = optuna.create_study(direction='maximize', study_name=f"{model_name}_hpo")
        objective_func = objective_builder(
            model_name, train_loader, val_loader, 
            tabular_input_size=len(feature_cols_tab) if model_name == 'hybrid' else 0,
        ...
        print(f"Best trial for {model_name.upper()}: AUPRC = {study.best_value:.4f}")
