...
from sklearn.metrics import balanced_accuracy_score, roc_auc_score, precision_score, recall_score, f1_score, confusion_matrix
...
model_configs = {
    'log_reg': True, 'autogluon': True, 'xgb': True, 'rf': True,
    'svm': True, 'mlp': True, 'knn': True, 'asa_rule': True,
}

# --- Optimized Hyperparameters for Each Dataset ---
hpo_params_preop = {
    'log_reg': {'C': 0.19863},
    'xgb': {'n_estimators': 515, 'learning_rate': 0.01148, 'max_depth': 9, 'subsample': 0.7182, 'colsample_bytree': 0.6793, 'gamma': 0.01291},
...
hpo_params_intraop = {
    'log_reg': {'C': 141.3782},
    'xgb': {'n_estimators': 675, 'learning_rate': 0.0105, 'max_depth': 6, 'subsample': 0.8110, 'colsample_bytree': 0.6934, 'gamma': 2.3038},
...
hpo_params_combined = {
    'log_reg': {'C': 0.0006},
    'xgb': {'n_estimators': 1110, 'learning_rate': 0.0172, 'max_depth': 9, 'subsample': 0.8281, 'colsample_bytree': 0.7827, 'gamma': 1.7456},
    'rf': {'n_estimators': 597, 'max_depth': 17, 'min_samples_split': 10, 'min_samples_leaf': 7, 'max_features': 'sqrt'},
    'svm': {'C': 0.0001},
        ...
        for model_key, should_run in model_configs.items():
            ...
            for i, (test_data, train_data) in enumerate(splitter, 1):
                ...
                if model_key == 'log_reg':
                    ...
                    model = LogisticRegression(**params, class_weight='balanced', max_iter=10000, tol=0.01, random_state=RANDOM_STATE + i)
                    ...
                    try:
                        ...
                        predictor.fit(
                            train_data=train_df, time_limit=300, presets='best_quality',
                            num_cpus=8, ag_args_fit={'num_gpus': 1}
                        )
                    ...
                    model = RandomForestClassifier(**params, class_weight='balanced', n_jobs=-1, random_state=RANDOM_STATE + i)
                    model.fit(X_train_scaled, y_train)
                    y_pred = model.predict(X_test_scaled)
                    y_prob = model.predict_proba(X_test_scaled)[:, 1]
                    ...
                    model = LinearSVC(**params, class_weight='balanced', dual='auto', random_state=RANDOM_STATE + i, max_iter=5000)
                    model.fit(X_train_scaled, y_train)
                    ...
                    lr, n_layers, n_units, dropout_rate = params.get('lr', 0.001), params.get('n_layers', 2), params.get('n_units', 32), params.get('dropout_rate', 0.5)
                    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
                    
                    X_train_part, X_val_part, y_train_part, y_val_part = train_test_split(X_train_scaled, y_train, test_size=0.2, random_state=RANDOM_STATE + i, stratify=y_train)
                    ...
                    pos_weight = torch.tensor([np.sum(y_train_part == 0) / np.sum(y_train_part == 1) if np.sum(y_train_part == 1)>0 else 1.0], device=device)
                    ...
                    for epoch in range(5000):
                        ...
                        if (epoch + 1) % 10 == 0:
                            ...
                            if current_val_auc > best_val_auc:
                                best_val_auc, patience_counter, best_model_state = current_val_auc, 0, copy.deepcopy(model.state_dict())
                            else:
                                patience_counter += 1
                    ...
                    model.eval()
                    with torch.no_grad():
                        y_prob = torch.sigmoid(model(torch.FloatTensor(X_test_scaled).to(device))).cpu().numpy().flatten()
            ...
            if not df_results.empty:
                ...
                # --- MODIFIED: Save base results after the first model run ---
                if not base_results_saved:
                    # The 'base' model in the original script used 'y_pred_binary' to store true labels
